...
def load_image(collection, filepath: str, grayscale: bytes, md5: str, height: int, width:int):
    with open(grayscale, "rb") as image_file:
        base64_img = base64.b64encode(image_file.read())
    collection.db.collection.update(
            {"md5": md5}, {"$set": {"grayscale": base64_img, "height": height, "width": width, "insert_time": dt.datetime.utcnow() }}, upsert=False
    )
...
def load_result(**context):
    ...
    for downloaded_image in downloaded_images:
        if downloaded_image["success"]:
            ...
            load_image(collection, image_filepath, image_grayscale, image_md5, image_height, image_width)
